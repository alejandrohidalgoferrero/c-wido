<?php 
    ...
    for ($i = 1; $i <= $num_filas; $i++) {
        ...
        if(is_null($_POST['despues_usar_'.$i]))
        {
            $despues_usar="";
        }
        else
        {
            $despues_usar=$_POST['despues_usar_'.$i];
        }


        $sql="UPDATE `$nombre_tabla_registro` SET `antes_usar`='$antes_usar',`despues_usar`='$despues_usar' WHERE `id_check`='$id_check' AND `id`='$i'";
        $consulta = mysqli_query($conexion, $sql);
