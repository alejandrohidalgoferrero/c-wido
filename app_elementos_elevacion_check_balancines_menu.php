...
<h1 align="center">CHECK CAMBIO DE BALANCINES.</h1>
...
<div class="row mt-1 mb-1 ml-1 mr-1 pt-1 pb-1 pl-1 pr-1  justify-content-center">
        <form action="app_elementos_elevacion_check_balancines_generar.php" method="POST"> 
            <div class="col-4 pt-2 pb-2 pl-2 pr-2 " style= "border:muted 3px solid">               
                <input type="submit" value="REALIZAR NUEVO CHECK" class="btn btn-primary">
            </div>             
        </form>
</div>
    ...
    <table style="text-align: center;" align="center" border="1"class="table table-striped table-sm">
        <thead class="thead-dark">
            <tr>
                ...
                <th scope="col">Puente Grua<br></td>
                <th scope="col">Operación<br></td>
                <th scope="col">Linea<br></td>
        ...
        <tbody>
            ...
            $consulta = "SELECT * FROM check_balancines_registro  WHERE id='1' ORDER BY orden DESC";
        ...
        ?>    
                <?php echo '<td style="vertical-align: middle;">
                                <form action="app_elementos_elevacion_check_balancines.php" method="POST">  
                                    <input type="submit" value="Modificar" class="btn btn-info btn-sm">
                                    <input type="hidden" name="orden" id="orden" value="'.$row['orden'].'"/>
                                </form>                
                            </td>' ;?>

                <?php echo '<td style="vertical-align: middle;"><label for="normal cooler"  ></label>'. $row['fecha'].' ('. $row['turno'].')</td>' ;?>
                <?php echo '<td style="vertical-align: middle;"><label for="normal cooler"  ></label>'. $row['puente_grua'].'</td>' ;?>
                <?php echo '<td style="vertical-align: middle;"><label for="normal cooler"  ></label>'. $row['operacion'].'</td>' ;?>
                <?php echo '<td style="vertical-align: middle;"><label for="normal cooler"  ></label>'. $row['linea'].'</td>' ;?>
